...
const MOCK_RESPONSES = {
  'Skeptical': {
    success: true,
    result: `# Skeptical Lens Analysis for Auria
...
## Investment Recommendation (Skeptical View)
**No** - Pass at current $40M valuation. The combination of regulatory uncertainty, unproven team in fintech, and concerning unit economics suggests waiting for more traction or a lower entry point.`
  },
  'Contrarian': {
    success: true,
    result: `# Contrarian Lens Analysis for Auria
...
## Investment Recommendation (Contrarian View)
**Yes** - Invest at ≤$50M post. The market is overlooking Auria's unique positioning in the B2B financial infrastructure space, where barriers to entry are higher and customer relationships more durable than in consumer fintech.`
  },
  'Optimistic': {
    success: true,
    result: `# Optimistic Lens Analysis for Auria
...
## Investment Recommendation (Optimistic View)
**Strong Yes** - Invest at ≤$60M post. Auria represents a rare opportunity to capture the financial infrastructure layer in a rapidly digitizing region with significant barriers to entry for global competitors.`
  },
  'CFO': {
    success: true,
    result: `# CFO Lens Analysis for Auria
...
## Investment Recommendation (CFO View)
**Yes** - Invest at ≤$45M post; pass above $50M. The unit economics and capital efficiency metrics support the investment case, though the valuation should account for the extended payback period and currency risks.`
  },
  'Unified': {
    success: true,
...
app.use('/zapier', createProxyMiddleware({
  ...
  onProxyRes: (proxyRes, req, res) => {
    ...
    proxyRes.on('end', () => {
      ...
      try {
        // Get the lens type from the request
        const lensType = req.body.lensType;
        
        // Return the mock response for the specific lens type
        if (MOCK_RESPONSES[lensType]) {
          res.setHeader('Content-Type', 'application/json');
          res.end(JSON.stringify(MOCK_RESPONSES[lensType]));
          console.log(`Sent mock response for ${lensType} lens`);
        } else {
          // If lens type is not recognized, return an error
          res.setHeader('Content-Type', 'application/json');
          res.status(400).end(JSON.stringify({
            success: false,
            error: `Unknown lens type: ${lensType}`
          }));
        }
      } catch (error) {
...
const PORT = 8005;
app.listen(PORT, () => {
  console.log(`Multi-lens proxy server running on port ${PORT}`);
