...
Conviction level: Medium. We have moderate conviction in this analysis – we are confident about the broad market opportunity but the uncertainties are significant: execution risks in emerging markets, the need to influence entrenched insurers, and competition from well-funded local peers all temper our enthusiasm.`
  ...
  const runAnalysis = async (data: { 
    ...
    // Extract plain text from any uploaded .docx files to include in prompts
    let mergedDocText = ''
    try {
      ...
      for (const f of data.files || []) {
        if (f.name && f.name.toLowerCase().endsWith('.docx')) {
          try {
            ...
            console.warn('Error extracting docx text for', f.name, err)
            // continue silently
          }
        }
      }
      ...
      if (docTexts.length) {
        mergedDocText = docTexts.join('\n\n---\n\n')
        // Limit size to avoid huge prompts (trim to first 15000 chars)
        if (mergedDocText.length > 15000) mergedDocText = mergedDocText.slice(0, 15000) + '\n\n[TRUNCATED]'
      }
    } catch (err) {
      console.error('Erro ao processar arquivos .docx:', err)
    }
    ...
    for (let i = 0; i < lenses.length; i++) {
      ...
      try {
        ...
        // Append extracted docx text to the prompt if available
        const docFragment = mergedDocText ? `\n\nDocument content (first ${mergedDocText.length} chars):\n${mergedDocText}` : ''

        const composedPrompt = `${lens.prompt}\n\n${docFragment}\n\nIMPORTANT: Format the response in HTML format.`
